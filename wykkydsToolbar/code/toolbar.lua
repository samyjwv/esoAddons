...
_addon.Feature.Toolbar.MakeSpacerControl = function(o)
	return _addon.Frames.__NewImage("wykkydsToolbar_Spacer", o)
		:SetTexture("/esoui/art/buttons/checkbox_mouseover.dds")
	...
	for k,t in ipairs(_addon.G.BAR_TOOLS) do
		...
		and _addon.TSetting(t.Tool) ~= false then
			...
			if not lastControl then
				...
				o.Tools[t.Tool].Control.padding =4
...
local updateAll = function()
	...
	if _addon:GetOrDefault( false, _addon.Settings["let_leave_screen"] ) ~= true and not _addon.Feature.Toolbar.IsMoving then
		...
		if gCenterX ~= nil and gCenterY ~= nil then
			if wykkydsToolbar:GetTop() <= 0 then   -- added "=" 5-21-15  This is the primary check to move compass down if feature is enabled.  Wasnt moving down as top=0
...
_addon.Feature.Toolbar.Create = function()
	...
	--if not groupTimers or (not _addon:GetOrDefault( false, _addon.Settings["horse_trainFull"] )) then
	...
	table.insert(_addon.G.BAR_TOOLS, { Tool = _addon.G.BAR_TOOL_BAGS, 	Method = _addon.Feature.Toolbar.GetBagDetails })
	...
	--if groupTimers or (not _addon:GetOrDefault( false, _addon.Settings["horse_trainFull"] )) then
	if groupTimers then
		table.insert(_addon.G.BAR_TOOLS, { Tool = _addon.G.BAR_TOOL_HORSE, 	Method = _addon.Feature.Toolbar.GetHorse })
	end
	...
	o.Tools = {
		[_addon.G.BAR_TOOL_BAGS] = { Name = "bag_setting", Control = nil },
		...
		[_addon.G.BAR_TOOL_GOLD] = { Name = "gold_setting", Control = nil },
