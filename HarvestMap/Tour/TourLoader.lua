...
function Loader:Initialize(fragment)
	...
	Harvest.savedVars["tours"] = ZO_SavedVars:NewAccountWide("Harvest_SavedVars", 1, "tours", {} )
...
function Loader:GetSavedTours()
	local tours = Harvest.savedVars["tours"][Harvest.mapPins.mapCache.map]
	if not tours then
		tours = {}
		Harvest.savedVars["tours"][Harvest.mapPins.mapCache.map] = tours
	end
	return tours
end
